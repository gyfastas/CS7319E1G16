...
class Normalize(torch.nn.Module):
    ...
    def forward(self, x):
        return torch.nn.functional.normalize(x, self.p, self.dim, self.eps)
...
class CatMerger(torch.nn.Module):
    def __init__(self):
        super().__init__()
...
def shift_indices_tuple(indices_tuple, batch_size):
    ...
    if len(indices_tuple) == 3:
        indices_tuple = (
            indices_tuple[0],) + tuple([x+batch_size if len(x) > 0 else x for x in indices_tuple[1:]])
    elif len(indices_tuple) == 4:
        indices_tuple = tuple([x+batch_size if len(x) > 0 and i %
                               2 == 1 else x for i, x in enumerate(indices_tuple)])
...
def latest_version(folder, string_to_glob="trunk_*.pth", best=False):
    ...
    if items == []:
        return (0, None)
    model_regex = regex_wrapper(
        "best[0-9]+\.pth$") if best else regex_wrapper("[0-9]+\.pth$")
...
def knn_dot_self(embeddings, K, return_distance=False):
    """
    KNN using dot similarity and remove my self
    Args:
        embeddings: torch.Tensor([N, F])
        K:
    """
    dot_sim = torch.matmul(embeddings, embeddings.T) # [N, N]
    diag_idx = range(dot_sim.shape[0])
    dot_sim[diag_idx, diag_idx] = -np.inf
    sim, idxs = dot_sim.topk(K, dim=1)
    if return_distance:
        return sim, idxs
    else:
        return idxs
